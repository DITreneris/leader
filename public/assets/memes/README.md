...
- `meme-01-ai-chaos.png` — **not mounted** (removed 2026-09-09); asset kept for optional reuse / social.
- `meme-02-meeting-overload.png` — **not mounted** on the live page (removed 2026-04-29); asset kept for optional reuse / social.
- `meme-03-clear-decision.png` — **not mounted** (removed 2026-09-09); asset kept for optional reuse / social.
- `meme-04-delegation-bottleneck.png` — adding AI to an unclear process only makes the bottleneck more visible.

Placement on the landing page (see `Page.astro`) — **two** `MemeMoment` beats, indices into `copy.memes.items`:

1. After `PromoBanner`, before `ClarityDemo` — `meme-04-delegation-bottleneck.png` (`index={2}`, gold).
2. Immediately after `ClarityDemo` — `reward_2.png` (`index={5}`, gold).

Not mounted: `memes.items[1]` (spare copy); `memes.items[3]` + `meme-03-clear-decision.png` and `memes.items[0]` + `meme-01-ai-chaos.png` removed **2026-09-09** (time-to-copy); `memes.items[4]` + `meme-02-meeting-overload.png` removed **2026-04-29** (see [`docs/MEME_PRIORITY_REGISTRY.md`](../../../docs/MEME_PRIORITY_REGISTRY.md)).
