...
|-------|---------|------|----------------------|
| 1 | 2 | `meme-04-delegation-bottleneck.png` | Delegation bottleneck |
| 2 | 5 | `reward_2.png` | Keep the structure on your desk |
...
Section flow: Hero → `ExecutiveModules` (`#context`) → `PromoBanner` → **meme index 2** → `ClarityDemo` (`#demo`) → **meme index 5** → `SafetyCheck` → `CourseCTA` (`#kit`) → `PromptAnatomy` → `RoiPath` → `Faq` → `PromptLibrary` (`#library`).

Removed **2026-09-09:** pre-context memes `items[3]` (`meme-03-clear-decision.png`) and `items[0]` (`meme-01-ai-chaos.png`) — `#context` follows the hero. See [`MEME_PRIORITY_REGISTRY.md`](MEME_PRIORITY_REGISTRY.md).

Removed **2026-04-29:** post–`RoiPath` meme `items[4]` (`meme-02-meeting-overload.png`).

Index `1` in `memes.items` is not mounted on the page today (reserved / spare).

Implementation rules:

- Raster delivery: **`MemeMoment`** emits `<picture>` with **AVIF → WebP → PNG** (PNG is the authoring master; run **`npm run optimize:memes`** after changing a meme PNG). Remaining beats are below the fold and stay lazy.
