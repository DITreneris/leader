...
- **Deploy origin fallback hardening:** when `SITE_URL` is missing, [`scripts/lib/deploy-env.mjs`](scripts/lib/deploy-env.mjs) now infers the site origin from hosting-provided env vars (e.g. `VERCEL_URL`) and [`astro.config.mjs`](astro.config.mjs) consumes that helper — prevents canonicals, **`og:image`** URLs, and the emitted `robots.txt` sitemap from defaulting to GitHub Pages on non-GitHub deploys.
...
- **SEO/GEO/AI anti-cannibalization split:** [`docs/SOURCE_OF_TRUTH.md`](docs/SOURCE_OF_TRUTH.md) now documents explicit intent boundaries (`leader` executive decision-ops vs mother product authority vs sister learning hub) plus allowed/avoid terms. [`docs/UTM_MATRIX.md`](docs/UTM_MATRIX.md) adds CTA intent guardrail. EN/LT copy in [`src/content/locales/en.ts`](src/content/locales/en.ts) / [`src/content/locales/lt.ts`](src/content/locales/lt.ts) was tightened toward decision-ops semantics (`meta`, hero, FAQ, promo, kit CTA). [`public/llms.txt`](public/llms.txt) now includes search-intent boundaries for AI systems. [`src/utils/pageJsonLd.ts`](src/utils/pageJsonLd.ts) adds executive decision-ops `WebPage.keywords`. [`docs/QUALITY_ASSURANCE.md`](docs/QUALITY_ASSURANCE.md) adds checks for intent split and CTA hierarchy.

- **PromoBanner sister hub path:** [`PromoBanner.astro`](src/components/PromoBanner.astro) now includes a tertiary text CTA to `promptanatomy.cloud` while preserving the primary handoff to PromptAnatomy.app and the in-page clarity-practice CTA. Added tracked builder [`buildSisterHubUrl`](src/constants/outboundLinks.ts) (`utm_medium=modules`, `utm_campaign=sister_hub`) and updated EN/LT locale keys `promoBanner.sisterCta` in [`en.ts`](src/content/locales/en.ts) / [`lt.ts`](src/content/locales/lt.ts). Docs: [`docs/UTM_MATRIX.md`](docs/UTM_MATRIX.md).

- **Meme visual consistency:** [`meme-01-ai-chaos.png`](public/assets/memes/meme-01-ai-chaos.png) re-exported in-repo to **1600×900** (16:9, brand `#0B1320` letterboxing via `sharp`) to match other mounted memes; regenerated **AVIF/WebP**. [`MemeMoment.astro`](src/components/MemeMoment.astro) — when **`side="right"`**, `lg` grid is **`0.75fr | 1.25fr`** so the image column stays **wide** (fixes image incorrectly using the narrow column). Docs: [`public/assets/memes/README.md`](public/assets/memes/README.md), [`docs/VISUAL_CONTENT_MAP.md`](docs/VISUAL_CONTENT_MAP.md).
...
- **Social preview hardening (root redirect + Facebook):** [`src/pages/index.astro`](src/pages/index.astro) now includes explicit root-level `og:*`/`twitter:*` tags (`og:url`, `og:title`, `og:description`, `og:image`) so crawlers that scrape `/` no longer infer metadata from the redirect shell (`Redirecting…`). Added optional `fb:app_id` emission in both [`src/pages/index.astro`](src/pages/index.astro) and [`src/layouts/Page.astro`](src/layouts/Page.astro) via env var `PUBLIC_FB_APP_ID`; documented in [`.env.example`](.env.example). Deploy target for the mirror is `SITE_URL=https://promptanatomy.pro`, `BASE_PATH=/`.
...
- **Domains (canonical vs Vercel mirror):** [`docs/SOURCE_OF_TRUTH.md`](docs/SOURCE_OF_TRUTH.md) — subsection **Domains (canonical product vs optional deploy)** (`www.promptanatomy.app` vs optional `promptanatomy.pro` hosting); one-line pointer in [`AGENTS.md`](AGENTS.md) Architecture; [`docs/README.md`](docs/README.md) index row for **SOURCE_OF_TRUTH**.
...
- **Measure-before-cut (anchor + nav canon):** [`docs/CODEBASE_OVERVIEW.md`](docs/CODEBASE_OVERVIEW.md) — **Hash anchors (registry)** table (`#top`, `#context`, `#demo`, `#safety-check`, `#kit`, `#anatomy`, `#roi`, `#faq`, `#library`, skip `#ctx-company`); **Hero nav** policy (primary ladder only; extra `nav.*` locale keys optional). [`docs/SOURCE_OF_TRUTH.md`](docs/SOURCE_OF_TRUTH.md) — pointer to that registry. [`docs/MOBILE_UI_AUDIT.md`](docs/MOBILE_UI_AUDIT.md) — `#roi` row + link to registry. [`docs/STRATEGIC_REVISION_PLAN.md`](docs/STRATEGIC_REVISION_PLAN.md) — ship baseline line lists `#anatomy` / `#roi` / `#faq` / `#library`.
...
- **Paste-after-copy (AI handoff):** [`PasteDestinationStrip.astro`](src/components/ds/PasteDestinationStrip.astro) — compact “after you copy, open your approved AI tool and paste” plus outbound **ChatGPT / Claude / Gemini** links and a license note. Wired after the module grid + custom card in [`ExecutiveModules.astro`](src/components/ExecutiveModules.astro) (`surface=context`) and after the demo copy + tab row in [`ClarityDemo.astro`](src/components/ClarityDemo.astro) (`surface=demo`, tight spacing). [`buildConsumerAiUrl`](src/constants/outboundLinks.ts) — `utm_medium` **`context_paste`** \| **`demo_paste`**, `utm_campaign` **chatgpt** \| **claude** \| **gemini**. EN/LT **`pasteIntoAssistant.*`** in [`en.ts`](src/content/locales/en.ts) / [`lt.ts`](src/content/locales/lt.ts). [`docs/UTM_MATRIX.md`](docs/UTM_MATRIX.md) — paste strip rows.

### Removed

- **Meme beat `memes.items[4]`:** Removed post–`RoiPath` [`MemeMoment`](src/components/MemeMoment.astro) (`meme-02-meeting-overload.png`); trimmed **`memes.sequenceImageAlts`** in [`en.ts`](src/content/locales/en.ts) / [`lt.ts`](src/content/locales/lt.ts) to four entries. PNG kept under [`public/assets/memes/`](public/assets/memes/) for optional reuse. Docs: [`MEME_PRIORITY_REGISTRY.md`](docs/MEME_PRIORITY_REGISTRY.md), [`VISUAL_CONTENT_MAP.md`](docs/VISUAL_CONTENT_MAP.md), [`CODEBASE_OVERVIEW.md`](docs/CODEBASE_OVERVIEW.md), [`STRATEGIC_REVISION_PLAN.md`](docs/STRATEGIC_REVISION_PLAN.md), [`COPY_AUDIT_BY_SLIDE.md`](docs/COPY_AUDIT_BY_SLIDE.md).
...
- **Standalone Today’s workflow section:** Removed [`TodaysWorkflow.astro`](src/components/TodaysWorkflow.astro), [`MiniWorkflowSchema.astro`](src/components/ds/MiniWorkflowSchema.astro), locale **`todayWorkflow.*`**, and **`buildBeforeAfterWorkflowSearchParams`** from [`outboundLinks.ts`](src/constants/outboundLinks.ts). Paste-into-assistant UX is covered by **`PasteDestinationStrip`** adjacent to copy actions (see **Added**).

### Changed

- **GEO / AI (`llms.txt`):** [`public/llms.txt`](public/llms.txt) — **Audience / market** section (default `/en/` US-market copy and tone; optional `/lt/` when bilingual ships) and **Notes for AI systems** bullet for US-primary audience citations.

- **RoiPath deep link + a11y:** [`RoiPath.astro`](src/components/RoiPath.astro) — outer `<section id="roi" tabindex="-1" aria-labelledby="roi-heading">`; [`SectionHeader`](src/components/SectionHeader.astro) `titleId="roi-heading"` so hash navigation and screen readers target the section consistently.
...
- **Weekly ROI path — five moves (~5h/week):** Merged former steps **Check before you send** + **Send a clear message** into one **`outbound`** step (**Review & send clearly**, 15–40 min) with a combined risk-pass + rewrite prompt. EN/LT [`roiPath.*`](src/content/locales/en.ts); [`RoiPath.astro`](src/components/RoiPath.astro) shows **Safety Check** for `outbound` and defaults progress to **Step X of 5**. Kit PDF/HTML weekly list first bullet aligned; rebuilt [`public/assets/downloads/executive-operating-kit.pdf`](public/assets/downloads/executive-operating-kit.pdf).

- **Copy tightening (scan density):** [`en.ts`](src/content/locales/en.ts) / [`lt.ts`](src/content/locales/lt.ts) — shorter **`meta.description`**; FAQ answers for paste / Executive OS / Global Context Block; **`heroArtifact.helper`**; **`pasteIntoAssistant.lead`** + **`licenseNote`**; **`modules.startHere`** + **`contextFootnote`**. LT em dashes normalized to **en dash** where edited (see CC-4 in [`COPY_AUDIT_BY_SLIDE.md`](docs/COPY_AUDIT_BY_SLIDE.md)).

- **Visual clarity iteration (SaaS scan path):** [`ExecutiveModules.astro`](src/components/ExecutiveModules.astro) — [`ContextFieldMap.astro`](src/components/ds/ContextFieldMap.astro) above the global context card (four fields → inject chip); single **`modules.contextFootnote`** replaces `contextWarning` + `contextCopyHint`. [`PromptLibrary.astro`](src/components/PromptLibrary.astro) — one **`library.summaryLead`** line replaces `scanHint` + `instructions`. [`PromptAnatomy.astro`](src/components/PromptAnatomy.astro) — closed-state **preview chips** for the five block titles + `anatomy.previewAriaLabel`. EN/LT in [`en.ts`](src/content/locales/en.ts) / [`lt.ts`](src/content/locales/lt.ts): `modules.contextMapAriaLabel`, `contextMapInjectLabel`, `contextMapCaption`; shorter **`modules.subtitle`**; docs [`COPY_AUDIT_BY_SLIDE.md`](docs/COPY_AUDIT_BY_SLIDE.md) §5/§13/§17, [`DESIGN_SYSTEM.md`](docs/DESIGN_SYSTEM.md) §2 (`ContextFieldMap`).

- **HeroBento (system UI):** [`HeroBento.astro`](src/components/HeroBento.astro) — unified row pattern (neutral fill, fixed `min-height`, `.radius-sm`); semantic **left border** + icon only (brand / rose / emerald); **1–3** step markers + `aria-posinset` / `aria-setsize` on the ordered list; `panelOverline` micro-label; **removed** per-row `surface-accent` / `accent-glow` and the second blur orb. EN/LT [`heroBento`](src/content/locales/en.ts): `panelOverline`, tightened `ariaLabel` / `tilesAriaLabel`; dropped unused `chip*`, `flow*`, `tileOwner`, `tileDeadline`. [`docs/DESIGN_SYSTEM.md`](docs/DESIGN_SYSTEM.md) — HeroBento bullet under §3 templates.

- **Clarity demo (`#demo`) interaction model:** [`ClarityDemo.astro`](src/components/ClarityDemo.astro) — replaced two stacked `<details>` (Show details / Show input) with a **single secondary panel** under a stable action row: **Copy prompt** + **Details** / **Input** tabs (`[data-demo-tab]`, `role="tablist"`). Default state shows a short **placeholder** (`demo.secondaryPanelHint`); **re-tapping the active tab** collapses back to the placeholder. Tighter brief card padding (`gap-3` / `p-4`). **Download Max Value Kit** in the follow-up band uses a **bordered text-style** link instead of the gold gradient CTA so it does not compete with **Copy prompt**. EN/LT: `detailsTab`, `inputTab`, `secondaryPanelHint`, `secondaryTablistAriaLabel`; removed `showDetails` / `hideDetails` / `showInput` / `hideInput`. [`InteractiveCopy.astro`](src/components/InteractiveCopy.astro) — `setDemoSubpanel` + `aria-selected` / `tabIndex` for the demo tablist; `renderScenario` still updates `[data-demo-field]`, `[data-demo-risks]`, `[data-demo-questions]` with an open tab unchanged. [`docs/MOBILE_UI_AUDIT.md`](docs/MOBILE_UI_AUDIT.md) — interaction table row for scenario selection notes the tabbed secondary panel.
