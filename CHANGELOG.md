...
- **Hero CTAs (conversion ladder):** [`Hero.astro`](src/components/Hero.astro) — gold primary stays **Open PromptAnatomy.app** with **`utm_campaign=primary`**; added outline secondary to **`#context`** (`hero.secondaryCta`). Supersedes earlier “hero row = single outbound button only” intent—matches [`.cursor/rules/visual-and-copy.mdc`](.cursor/rules/visual-and-copy.mdc) in-page ladder. [`docs/UTM_MATRIX.md`](docs/UTM_MATRIX.md): hero primary row + in-page secondary note.
...
- **`MemeMoment` layout:** Full-width **16:9** image **above** centered title + caption on all breakpoints (removed `side` / two-column `lg` grid); [`Page.astro`](src/layouts/Page.astro) drops `side` props. Docs: [`docs/VISUAL_CONTENT_MAP.md`](docs/VISUAL_CONTENT_MAP.md), [`public/assets/memes/README.md`](public/assets/memes/README.md).
