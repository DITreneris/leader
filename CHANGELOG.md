...
- **Today's workflow + mini schema:** [`TodaysWorkflow.astro`](src/components/TodaysWorkflow.astro) — single centered **`rounded-3xl`** gradient + blur shell (header + schema + `#context` CTA band separated by a hairline + text-style ChatGPT / Claude / Gemini links + license note); removed two-column layout, inner **`body`/`hint`** stack, `HighlightStrip` hint band, and pill borders on outbound links (divider-separated inline links). [`MiniWorkflowSchema.astro`](src/components/ds/MiniWorkflowSchema.astro) — **four** steps (**Build → Copy → Paste → Decide**) as numbered badge + label only (no per-step card chrome / pipeline track ring); vertical stack on mobile, horizontal rail on `sm+`; **Decision brief** output as one soft gradient block (`outputTitle` / `outputDetail`). EN/LT **`todayWorkflow.*`**: `schemaStepBuild`, `schemaStepCopy`, `schemaStepPaste`, `schemaStepDecide`, `schemaOutputTitle`, `schemaOutputDetail`. [`pageSeo.ts`](src/constants/pageSeo.ts) — **`LEADER_PAGE_DATE_MODIFIED`** = **2026-04-29**.
